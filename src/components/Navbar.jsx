...
import { assets } from '../../public/images/assets/assets.js'
...
function Navbar() {
    ...
    // whenever the searchQuery changes, it will execute the function
    useEffect(()=>{
  ...
  return (
    <nav className="flex items-center justify-between px-4 md:px-14 border-b border-gray-300 bg-white relative transition-all">
            ...
            <div className="hidden sm:flex items-center gap-8">
                ...
                <div onClick={()=> navigate("/cart")} className="relative cursor-pointer">
                    <img src={assets.nav_cart_icon} alt="cart" className='w-6 opacity-88' />
                    <button className="absolute -top-2 -right-3 text-xs text-white bg-primary w-[18px] h-[18px] rounded-full">
                        {getCartCount()}
                    </button>
                </div>

                {/* This login button will be displayed only when the user is not logged in */}
            ...
            <div className='flex items-center gap-6 sm:hidden'>
                ...
                <button onClick={() => open ? setOpen(false) : setOpen(true)} aria-label="Menu" className="">
                    {/* Menu Icon SVG */}
                    {/* When we will click on this menu icon, it will hide and display the menu icon */}
            ...
            py-4 flex-col items-start gap-2 px-5 text-sm md:hidden`}>
                ...
                {/* the my orders will be displayed only if the user is logged in */}
                {user &&
                <NavLink to='/products' onClick={()=> setOpen(false)}>My Orders</NavLink>
                }
                <NavLink to='/' onClick={()=> setOpen(false)}>Contact</NavLink>

                {/* Whenever the user is not logged in */}
                {!user ? (
                    <button onClick={()=> {
                        setOpen(false);
